...
app.get('/law', async (req, res) => {
  try {
    ...
    // 국가법령정보 API URL 및 파라미터 준비
    const lawApiUrl = 'http://www.law.go.kr/DRF/lawService.do';
    const params = {
      OC: OC_VALUE,
      target: 'law',
      type: 'JSON',
      ID: id
      // 만약 필요하다면 MST를 추가할 수도 있습니다.
      // MST: '마스터번호'
    };
    ...
    // 조문 배열(JSON 키명은 실제 응답 구조에 맞춰 수정해주세요)
    // 예: lawRoot['조문번호'] 혹은 lawRoot['법령조문']
    ...
    console.error('Error in /law handler:', e);
